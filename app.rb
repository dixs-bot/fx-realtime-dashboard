...
helpers do
  # Ambil data 1M dari Twelve Data
...
get "/signal" do
  ...
  begin
    ...
    puts "[ERROR /signal] #{e.class}: #{e.message}"
    status 500
    { error: e.message }.to_json
  end
end

# ================== API /upload_chart + OpenAI ==================
...
post "/upload_chart" do
  begin
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: "Ini adalah screenshot chart trading (mungkin dari OlympTrade). " \
                    "Analisa dalam bahasa Indonesia: apakah market cenderung uptrend, downtrend, atau sideways, " \
                    "bagaimana momentum & volatilitas, dan berikan saran EDUKASI untuk latihan di akun demo (bukan saran finansial). Jawab singkat & jelas."
            },
            {
              type: "input_image",
              image_url: data_url
    ...
    # coba ambil teks dari field output, kalau struktur beda -> fallback ke JSON mentah
    ai_comment =
      if parsed["output"].is_a?(Array)
        first = parsed["output"].first
        if first && first["content"].is_a?(Array)
          text_block = first["content"].find { |c| c["type"] == "output_text" }
          text_block ? text_block["text"] : parsed.to_json
        else
          parsed.to_json
        end
      else
    ...
    puts "[ERROR /upload_chart] #{e.class}: #{e.message}"
...
.btn-refresh:hover {
  ...
  box-shadow: 0 0 18px var(--green), inset 0 0 10px rgba(0,255,136,0.7);
}
...
.pair-table-title { font-size: 16px; margin-bottom: 8px; }
.pair-table {
  width: 100%;
...
<div class="shell">
  ...
  <div class="card">
    <div class="signal-header-row">
      <div style="flex:1;">
        <h2 style="margin:0 0 4px 0;" id="titlePair">Signal EUR/USD (1M Realtime)</h2>
        <div class="label">
          Data: Twelve Data · Interval: 1M. Cocok untuk latihan di akun demo OlympTrade.
        </div>
      ...
      <div>
        <div class="label" style="margin-bottom:2px;">Timeframe Label (catatan saja):</div>
        <div class="tf-group">
  ...
  <div class="card mini-chart-card">
    <div class="label" id="tfLabel">
      History Candlestick · Interval: 1M · Pair: EUR/USD · TF Label: 1M  
      Tips: pinch/scroll untuk zoom · drag untuk geser kiri/kanan.
    </div>
  ...
  <div class="card ai-card">
    ...
    <p id="aiText" class="ai-text">
      Setelah data beberapa puluh candle terkumpul, AI akan memberi komentar:
      market lagi uptrend, downtrend, atau sideways, plus saran edukatif.
    </p>
    <p class="ai-note">
      Catatan: ini alat edukasi, bukan saran finansial.
    </p>
  </div>

  <!-- UPLOAD SCREENSHOT + AI VISION -->
  <div class="card">
    <h3 style="margin-top:0;">Upload Screenshot Market (OlympTrade)</h3>
    <p class="label">
      Upload screenshot chart dari OlympTrade di sini. Gambar akan dianalisa oleh AI OpenAI
      dan komentar akan muncul di bawah.
    </p>
    ...
    <p class="upload-note">
      Jangan upload data pribadi. Hanya screenshot chart saja.
    </p>
    #{if uploaded_img_url
...
async function ambilSignal(fromButton = false) {
  ...
  try {
    ...
    document.getElementById("titlePair").innerText =
      "Signal " + (data.pair_name || currentPair) + " (1M Realtime)";
...
function drawFullHistory(pairName) {
  ...
  tfLabel.innerText =
    "History Candlestick · Interval: 1M · Pair: " +
    pairName + " · TF Label: " + currentTF +
    " · Zoom: pinch/scroll, Pan: drag kiri/kanan";
  ...
  chartObj = new Chart(ctx, {
    ...
    data: {
      datasets: [{
        label: 'Price',
        data: values,
        color: { up:'#00ff55', down:'#ff1744', unchanged:'#9ca3af' },
        borderColor: { up:'#00cc44', down:'#d50032', unchanged:'#9ca3af' },
        borderWidth: 2,
        barThickness: 6,
        maxBarThickness: 8
      }]
...
function analyzeMarketAI(data){
  ...
  const badgeEl  = document.getElementById("aiTrendBadge");
  ...
  let trendBadge="SIDEWAYS", badgeClass="ai-badge badge-sideways",
      trendText="", rsiText="", volText="", eduText="";

  if (changePct > 0.6){
    trendBadge="UPTREND KUAT"; badgeClass="ai-badge badge-up";
    trendText="Market "+pair+" pada "+tf+" sedang uptrend cukup kuat (+"+changePct.toFixed(2)+"% dalam ±40 candle).";
  } else if (changePct > 0.2){
    trendBadge="UPTREND RINGAN"; badgeClass="ai-badge badge-up";
    trendText="Market cenderung bullish ringan. Arah naik ada tapi tidak terlalu agresif.";
  } else if (changePct < -0.6){
    trendBadge="DOWNTREND KUAT"; badgeClass="ai-badge badge-down";
    trendText="Market "+pair+" sedang downtrend kuat ("+Math.abs(changePct).toFixed(2)+"% turun dalam ±40 candle).";
  } else if (changePct < -0.2){
    trendBadge="DOWNTREND RINGAN"; badgeClass="ai-badge badge-down";
    trendText="Market cenderung bearish ringan. Tekanan turun ada.";
  } else {
    trendBadge="SIDEWAYS"; badgeClass="ai-badge badge-sideways";
    trendText="Market "+pair+" cenderung sideways / ranging.";
  }
  ...
  if (trendBadge.includes("UPTREND"))
    eduText="Latihan: fokus BUY di arah trend setelah koreksi, jangan kejar candle yang sudah sangat panjang.";
  else if (trendBadge.includes("DOWNTREND"))
    eduText="Latihan: fokus SELL setelah pullback gagal tembus resistance, hindari melawan trend.";
  else
    eduText="Latihan terbaik di sideways adalah belajar MENAHAN diri, tunggu breakout jelas.";

  badgeEl.className = badgeClass;
  badgeEl.innerText = trendBadge;
  ...
  buttons.forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      if (allCandles.length>0){
        drawFullHistory(document.getElementById("tblPair").innerText || currentPair);
      }
    });
  });
}
